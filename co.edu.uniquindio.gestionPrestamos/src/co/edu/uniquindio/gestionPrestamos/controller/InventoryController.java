...
public class InventoryController {
	...
	private ObservableList<Objeto> obtenerObjetos() {
		listaProductos.addAll(aplicacion.obtenerHabitaciones());
	...
	private void mostrarInformacionObjeto(Objeto producto) {
		if (producto != null) {
			txtnombreObjetoInventario.setText(producto.getTipoHabitacion());
    ...
    private void registarInventario() {
		...
		if (validarDatosObjeto(nombreObjeto, codigoObjeto,  estadoObjeto, valorObjeto, objetoTipo, unidadesDis, unidadesPres)) {

			int unidadesDisponibles = Integer.parseInt(unidadesDis);
			int unidadesPrestadas = Integer.parseInt(unidadesPres);
			Objeto objeto = null;
			objeto = aplicacion.registrarAObjeto(codigoObjeto, nombreObjeto, valorObjeto,
					estadoObjeto, objetoTipo, unidadesDisponibles, unidadesPrestadas);
    ...
    private void actualizarInventario() throws ProductExistException {
		...
		if (objetoSeleccionado != null) {
			if (validarDatosObjeto2(nameProduct, productCode, productWeight, conditionProduct, productValue,
				...
				aplicacion.actualizarHabitacion(objetoSeleccionado.getCodigo(), productCode, nameProduct, productValue, conditionProduct, objetoTipo,
	...
	private boolean validarDatosObjeto(String nameProduct, String productCode,
			String conditionProduct, String productValue,
		...
		if (productCode == null || productCode.equals("") || productCode.matches("[a-zA-Z]+")) {
			...
			if (objetoSeleccionado != null) {
				if (productCode.equals(objetoSeleccionado.getCodigo())) {
					if (aplicacion.verificarCodigoHabitacion(productCode)) {
